...
<div className="xl:w-[60%] w-full lg:p-10  lg:pb-0  p-5 pb-2 xl:text-left text-center  h-full">
  ...
  <button className='px-9 py-2 border border-black text-[14px] font-semibold bg-black hover:bg-transparent hover:text-black '>My Link Tree</button>
  <button className='px-9 py-2 border text-[14px] font-semibold border-black text-black hover:bg-black hover:text-white'>My Latest Articles</button>
  </div>
</div>
...
Hi! I&lsquo;m Vishal Solanki, a Fullstack Web Developer based in India.
</div>
</div>
<SkillsScrollbar/>
    </>
...
const SkillsScrollbar = ()=>{
  ...
  <div className='w-[10%] border-0 border-blue-500 items-top flex justify-start font-bold text-xl pt-1'>My Skills</div>
  <div className='flex flex-row justify-start gap-5 items-center w-[90%] overflow-y-hidden overflow-x-scroll pb-4 skill-section-custom-scrollbar'>
    {VISHAL_TECH_SKILLS?.map((skill)=>(
      <div key={skill} className='border-2 border-white-500 px-4 py-1 w-fit whitespace-nowrap hover:bg-white hover:text-black font-semibold'>
        {skill}
      </div>
    ))}
  </div>
